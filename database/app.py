...
from hash_function import find_word_in_buckets, hash_word
from read_file import read_file
from pages import mount_pages
from buckets import Bucket
import hash_function as hash_function
...
with st.sidebar:
    ...
    if st.button("Construir Índice"):
        ...
        st.session_state.metrics = run(lines_per_page=lines_per_page, bucket_size=bucket_size)
...
if st.session_state.metrics:        
    ...
    with col2:
        ...
        st.metric("Colisões (%)", f'{st.session_state.metrics["Percentage of collisions"]:.2f}%')
    with col3:
        st.metric("Total de Colisões", st.session_state.metrics["Total collisions"])
        st.metric("Overflows (%)", f'{st.session_state.metrics["Percentage of bucket overflows"]:.2f}%')
    ...
    if st.session_state.pages:
        ...
        # Primeira Página
        with col1:
            st.subheader(f"Página 1")
            first_page_df = pd.DataFrame(st.session_state.pages[0], columns=["Palavras"])
            st.table(first_page_df)

        # Última Página
        with col2:
            st.subheader(f"Página {len(st.session_state.pages)}")
            last_page_df = pd.DataFrame(st.session_state.pages[-1], columns=["Palavras"])
        ...
        if search_word:
            if st.session_state.buckets:
                ...
                if found_page is not None:
                    st.success(f"Palavra '{search_word}' encontrada na página {found_page + 1}")
                else:
                    st.warning(f"Palavra '{search_word}' não encontrada")
                
                # Exibição simplificada de custo e tempo
                st.info(f"Custo da busca com índice: 1 acesso ao disco (1 página lida)")
        ...
        if search_word:
            ...
            for page_index, page_words in enumerate(st.session_state.pages):
                ...
                if search_word in page_words:
                    ...
                    st.success(f"Palavra '{search_word}' encontrada na página {page_index + 1}")
            ...
            st.error("Por favor, digite uma palavra para realizar a Varredura de Tabela")
    ...
    if 'search_time' in st.session_state and 'table_scan_time' in st.session_state:
        ...
        st.write(f"Diferença de tempo (Varredura de Tabela - Busca com Índice): {st.session_state.table_scan_time - st.session_state.search_time:.6f} segundos")